...
Marking a field volatile (`set_volatility` on the field, or the corresponding attribute in whatever generates the model, see [RAL Generation in Practice](../ral-generation)) tells RAL not to trust its own mirrored value between accesses. A non-volatile RW field can be reasoned about purely from the model's state. If nobody has written it, its value hasn't changed. A volatile field has no such guarantee, and RAL's default handling reflects that: certain sequences re-read a volatile register rather than rely on the mirror, and the model won't flag a stale mirrored value against a fresh hardware read as an error the way it would for a non-volatile field.
