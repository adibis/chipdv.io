...
Neither flavor of this bug shows up in the message-ID logging this series builds starting a few articles from now, at least not by observation alone, the same way article 01's race eventually does. Cheap logging tells you a corruption happened and roughly when. Proving *why*, for a same-core temporal race, needs the interleaving itself to be forced or modeled, not just watched for.
